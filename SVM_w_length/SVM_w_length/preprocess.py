...
#	num_resamplings_2 = num_resamplings * num_resamplings 
	num_resamplings_2 = (num_resamplings + 1) * (num_resamplings + 1)

        # build an output array large enough to hold the vectors for each stroke and the (unicode -> int), the stroke_length, and stroke value (1 elts)
...
#		X_matrix = np.zeros((num_params, num_resamplings)) # the array to store in
		X_matrix = np.zeros((num_params, num_resamplings + 1)) # the array to store in, including stroke length
...
#		concatenated_X_X_2 = np.zeros((num_params, num_resamplings_2 + num_resamplings)) 
		concatenated_X_X_2 = np.zeros((num_params, num_resamplings_2 + num_resamplings + 1)) 
...
#                        curr_stroke[1] = curr_stroke_temp[1]
		...
		for j in range(0, num_params): # iterate through parameters
			...
			resampled_data = interpolate.splev(time_new, tck, der=0) # the resampled data
                        ...
                        # CONCATENATE STROKE LENGTH
                        resampled_data = np.concatenate((resampled_data, stroke_length))
