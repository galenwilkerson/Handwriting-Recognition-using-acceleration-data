...
NOTE THAT RADIUS OF CURVATURE IS FUNCTION OF FIRST AND SECOND DERIVATIVES
- also compute derivatives (1 and 2) 
...
#        output_array = np.zeros((num_strokes, (num_resamplings_2 + num_resamplings) * num_params + 1))
        output_array = np.zeros((num_strokes, (2 * num_resamplings) * num_params + 1))
...
#                        print float(i)/num_strokes
		...
		X_matrix = np.zeros((num_params, num_resamplings * 2)) # the array to store in (using original data and 2 derivs, 2 integrals)
...
#		concatenated_X_X_2 = np.zeros((num_params, num_resamplings_2 + num_resamplings)) 
		concatenated_X_X_2 = np.zeros((num_params, num_resamplings * 2)) # the array to store in (using original data and 2 derivs, 2 integrals)
...
#                        resampled_data_integral_2 = preprocessing.scale(resampled_data_integral_2)
                        ...
                        concatenated_resampled_data = np.concatenate((resampled_data_deriv, 
