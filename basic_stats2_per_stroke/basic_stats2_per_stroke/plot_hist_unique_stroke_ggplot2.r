...
for(param.to.plot in params.to.plot){
    ...
    # TIME SERIES
    title.name <- paste("Time_Series:_", param.to.plot, sep="_")
    file.name <- paste0(fig.dir,title.name,".pdf")
#    c <- ggplot(pen.data, aes(x = time, y = accel_x)) +
#    c <- ggplot(pen.data, aes(x = time, y = accel_x), color = label) +
#        geom_smooth()
    ...
    file.name <- paste0(fig.dir,title.name,".pdf")
    ggplot(data = subset(pen.data, Stroke_ID)) + 
#    pdf(file.name)
    qplot(acf(pen.data[[param.to.plot]], plot=FALSE, main = title.name))
#    dev.off()
